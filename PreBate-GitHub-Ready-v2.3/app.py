...
PRIMARY = "#10243D"; SUCCESS="#16A34A"; WARNING="#F59E0B"; DANGER="#DC2626"

st.markdown("""
<style>
  .hero { text-align:center; }
  .hero h1 { font-size: 2.2rem; margin: .25rem 0; color: #10243D; }
...
if LOGO_PATH is not None:
    try:
        ...
        st.markdown('<img alt="PreBate" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAEklEQVR4nGNgGAWjYBSMAggAAAQQAAFVN1rQAAAAAElFTkSuQmCC" style="width:100%;max-width:900px;">', unsafe_allow_html=True)
else:
    st.warning("Logo not found. Place a PNG at: " + ", ".join([str(p) for p in CANDIDATES]))
    st.markdown('<img alt="PreBate" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAEklEQVR4nGNgGAWjYBSMAggAAAQQAAFVN1rQAAAAAElFTkSuQmCC" style="width:100%;max-width:900px;">', unsafe_allow_html=True)
...
if st.session_state.completed:
    ...
    if a.get("q_property_coown") == "Yes" and a.get("q_property_joint_tenants") == "No": probate_risk += 1; add("Convert co-owned property to joint tenancy where appropriate to enable automatic survivorship.")
    ...
    if a.get("q_caregiver_access") == "Yes":
        if a.get("q_caregiver_official") == "Yes":
            ...
            dispute_risk += 2; probate_risk += 1; add("Revoke informal access (shared cards/PINs). Establish an Enduring Power of Attorney (EPA) if help is needed."); add("Keep a simple log of legitimate expenses paid by helpers on your behalf.")
    ...
    if a.get("q_will") == "No": probate_risk += 2; add("Create a valid will—without one, intestacy rules apply.")
    ...
    def build_pdf():
        ...
        if LOGO_PATH is not None:
            try:
                story.append(RLImage(str(LOGO_PATH), width=220, height=66))
            except Exception:
                pass
